...
class ObsPointNet(nn.Module):
    def __init__(self, input_dim: int = 2,  output_dim: int=4) -> None:
        ...
        self.MLP = nn.Sequential(   nn.Linear(input_dim, hidden_dim),   # 层1：2 → 32
                                    nn.LayerNorm(hidden_dim),           # 归一化
                                    nn.Tanh(),
                                    nn.Linear(hidden_dim, hidden_dim),  # 层2：32 → 32
                                    nn.ReLU(),
                                    nn.Linear(hidden_dim, hidden_dim),  # 层3：32 → 32
                                    nn.LayerNorm(hidden_dim),
                                    nn.Tanh(),
                                    nn.Linear(hidden_dim, hidden_dim),  # 层4：32 → 32
                                    nn.ReLU(),
                                    nn.Linear(hidden_dim, hidden_dim),  # 层5：32 → 32
                                    nn.LayerNorm(hidden_dim),
                                    nn.Tanh(),
                                    nn.Linear(hidden_dim, output_dim),  # 输出层：32 → 4
                                    nn.ReLU(),
                                    )
